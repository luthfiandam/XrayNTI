...
_Generated via Sistem Monitoring Pemeliharaan_`;
  ...
  return (
    <div className="space-y-4">
      {/* Top Banner Card */}
      <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-indigo-950 text-white rounded-2xl p-4 sm:p-5 shadow-lg border border-slate-700/60 relative overflow-hidden">
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <span className="text-[11px] font-extrabold uppercase tracking-wider bg-blue-500/30 text-blue-300 border border-blue-400/30 px-2.5 py-0.5 rounded-full">
                Supervisor Daily Brief
              </span>
              <span className="text-xs text-slate-300 font-medium">
                Shift {shift} ({shift === 'Malam' ? 'M' : 'PS'})
              </span>
            </div>
            ...
            <div className="flex items-center gap-2 text-xs text-slate-300 flex-wrap pt-0.5">
              <span className="font-semibold text-slate-400">Petugas On Duty:</span>
              {technicianNames.length > 0 ? (
                technicianNames.map((t, idx) => (
                  <span key={idx} className="bg-white/10 px-2 py-0.5 rounded text-white font-medium">
                    {t}
                  </span>
                ))
              ) : (
                <span className="text-amber-300">Belum diset</span>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <button
              ...
              className="px-3.5 py-2.5 bg-emerald-600 hover:bg-emerald-500 active:bg-emerald-700 text-white font-bold rounded-xl text-xs flex items-center gap-2 transition-all cursor-pointer shadow-md shadow-emerald-950/40"
            >
            ...
            <button
              ...
              className="px-4 py-2.5 bg-sky-500 hover:bg-sky-400 active:bg-sky-600 text-white font-bold rounded-xl text-xs flex items-center gap-2 transition-all cursor-pointer shadow-md shadow-sky-950/40 disabled:opacity-60"
            >
            ...
            {onOpenTelegramModal && (
              <button
                ...
                className="p-2.5 bg-white/10 hover:bg-white/20 text-white rounded-xl text-xs flex items-center justify-center transition-all cursor-pointer"
              >
      ...
      >
        ...
        >
          ...
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-xs text-slate-500 font-semibold">/ {totalScheduled} selesai</span>
          </div>
        ...
        <div
          ...
          className="bg-white border border-slate-200/90 rounded-2xl p-4 shadow-xs space-y-2.5 hover:border-rose-400 transition-colors cursor-pointer group"
        >
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-600 uppercase tracking-wide flex items-center gap-1.5">
          ...
          <div className="flex items-baseline gap-1.5">
            <span className="text-2xl font-black text-slate-900">{activeCorrectives.length}</span>
